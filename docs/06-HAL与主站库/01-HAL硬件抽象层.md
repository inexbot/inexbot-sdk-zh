...
| 参数说明 | device_type：设备类型<br>device_name：设备名称 |
| 返回值 | 无 |
| 备注说明 | 用于创建Device对象，并初始化其设备类型和名称。 |
...
| 功能描述 | 从设备管理器中移除一个设备。 |
| 参数说明 | device_type：设备的类型<br>device_name：要移除的设备的名称 |
| 返回值 | 0表示成功，非0表示失败 |
...
| 参数说明 | device_type：设备的类型<br>device_name：要检查的设备名称 |
...
| 参数说明 | device_type：要获取名称列表的设备类型 |
| 返回值 | 包含指定类型设备名称的字符串向量 |
...
| 参数说明 | device_type：要获取列表的设备类型 |
| 返回值 | 包含指定类型设备对象的共享指针向量 |
...
| 参数说明 | device_type：设备的类型<br>device_name：要获取的设备名称 |
| 返回值 | 指定类型和名称设备的共享指针。如果设备不存在，则返回 nullptr |
...
| 功能描述 | 设置指定通道的模拟输出值。 |
| 参数说明 | channel：模拟输出通道的编号<br>value：要设置的模拟输出值 |
| 返回值 | 无 |
...
| 功能描述 | 设置多个通道的模拟输出值。 |
| 参数说明 | channels：包含要设置的模拟输出通道编号的向量<br>values：与channels向量对应的模拟输出值的向量 |
| 返回值 | 无 |
...
| 功能描述 | 获取指定通道的模拟输入值。 |
| 参数说明 | channel：模拟输入通道的编号 |
| 返回值 | double类型的值，表示指定通道的模拟输入值 |
...
| 功能描述 | 获取多个通道的模拟输入值。 |
| 参数说明 | channels：包含要获取的模拟输入通道编号的向量 |
| 返回值 | std::vector<double>类型的值，包含指定通道的模拟输入值 |
...
| 功能描述 | 获取指定通道的量程范围。 |
| 参数说明 | channel：模拟I/O通道的编号 |
| 返回值 | std::pair<double, double>类型，表示量程范围（最小值和最大值） |
...
| 参数说明 | channel：指定要设置波特率的CAN设备通道号<br>baudrate：要设置的波特率值 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要设置接收滤波器ID的CAN设备通道号<br>enable_recv_filter：是否启用接收滤波器<br>recv_filter_id：要设置的接收滤波器ID值 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要设置帧格式的CAN设备通道号<br>format：要设置的帧格式类型 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要检测功能的CAN设备通道号<br>function：要检测的功能类型 |
| 返回值 | true表示支持该功能，false表示不支持 |
...
| 功能描述 | 发送指定通道的CAN报文。 |
| 参数说明 | channel：指定要发送报文的CAN设备通道号<br>txid：发送报文的ID<br>buff：发送报文的数据缓冲区<br>len：发送报文的数据长度<br>use_remote_frame：是否发送远程帧，默认为false |
| 返回值 | 无 |
...
| 功能描述 | 接收指定通道的CAN报文。 |
| 参数说明 | 输入参数channel：指定要接收报文的CAN设备通道号<br>输出参数rxid：接收报文的ID<br>输出参数buff：接收报文的数据缓冲区 |
| 返回值 | unsigned int类型的值，表示接收到的报文长度 |
...
| 参数说明 | channel：指定要设置的数字IO通道编号<br>state：输出状态，true表示高电平，false表示低电平 |
| 返回值 | 无 |
...
| 参数说明 | channels：包含要设置的数字IO通道编号的向量<br>states：包含每个数字IO通道的输出状态的向量 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要获取的数字IO通道编号 |
| 返回值 | true表示高电平，false表示低电平 |
...
| 参数说明 | channels：包含要获取的数字IO通道编号的向量 |
| 返回值 | std::vector<bool>类型的值，向量中的每个元素表示对应通道的输入状态 |
...
| 项目 | 说明 |
|------|------|
| 函数原型 | virtual int64_t get_encoder_value() const; |
| 功能描述 | 获取编码器的值。 |
| 参数说明 | 无 |
| 返回值 | int64_t类型的值，表示编码器的当前值 |

#### get_encoder_range

| 项目 | 说明 |
|------|------|
| 函数原型 | virtual std::pair<int64_t, int64_t> get_encoder_range() const; |
| 功能描述 | 获取编码器的范围。 |
| 参数说明 | 无 |
| 返回值 | std::pair<int64_t, int64_t>类型，pair的第一个元素表示编码器的最小值，第二个元素表示最大值 |

#### get_encoder_direction

| 项目 | 说明 |
|------|------|
| 函数原型 | virtual int get_encoder_direction() const; |
| 功能描述 | 获取编码器的方向。 |
| 参数说明 | 无 |
| 返回值 | 1表示正转，-1表示反转 |
...
| 参数说明 | channel：指定要打开的Serial设备通道号 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要关闭的Serial设备通道号 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要设置的Serial设备通道号<br>baudrate：要设置的波特率值<br>parity：校验位类型，"N"无校验，"E"奇校验，"O"偶校验<br>stop_bits：停止位 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要发送报文的Serial设备通道号<br>data：需要发送的数据<br>bytes_to_send：发送报文的数据长度 |
| 返回值 | 无 |
...
| 参数说明 | channel：指定要接收报文的Serial设备通道号<br>输出参数buffer：接收报文的缓存<br>输出参数bytes_received：接收报文的长度 |
| 返回值 | unsigned int类型的值，表示接收到的报文长度 |
